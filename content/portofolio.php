<?php
$queryCategoties = mysqli_query($koneksi, "SELECT * FROM categories WHERE type='portofolio' ORDER BY id DESC");
$rowCategories = mysqli_fetch_all($queryCategoties, MYSQLI_ASSOC);

$queryPortofolios = mysqli_query($koneksi, "SELECT DISTINCT categories.name, categories.id as id_category, portofolios.* FROM portofolios LEFT JOIN categories ON categories.id = portofolios.id_category WHERE is_active=1 ORDER BY portofolios.id DESC");
